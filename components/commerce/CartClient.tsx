...
export function CartClient() {
  ...
  const shipping = isPaymentTestOrder || subtotal >= 50000 || subtotal === 0 ? 0 : 3000;
  ...
  return (
    <div className="shell cart-page">
      ...
      <div className="cart-layout">
        ...
        <aside className="order-summary"><h2>주문 금액</h2><dl><div><dt>상품 금액</dt><dd>{formatPrice(subtotal)}</dd></div><div><dt>배송비</dt><dd>{shipping === 0 ? "무료" : formatPrice(shipping)}</dd></div><div className="summary-total"><dt>결제 예정 금액</dt><dd>{formatPrice(subtotal + shipping)}</dd></div></dl><Link className="add-button" href="/checkout">주문하기</Link><p>{isPaymentTestOrder ? "테스트 결제 상품은 배송비가 없습니다." : "5만 원 이상 구매 시 배송비가 무료입니다."}</p></aside>
      </div>
    </div>
  );
}

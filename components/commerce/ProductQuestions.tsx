...
import { CaretDown, Check, ChatCircleDots, CheckCircle, LockKey, Package, Palette, PencilSimple, Trash, X } from "@phosphor-icons/react";
...
export function ProductQuestions({ productSlug, productName }: { productSlug: string; productName: string }) {
  ...
  const toggleAnswer = (questionId: string) => setOpenAnswerIds((current) => {
      ...
      <div className="question-content">
        ...
        <div className="question-list" aria-busy={!ready}>
          ...
          {ready && questions.map((question) => <article className="question-item" key={question.id}>{editingId === question.id ? <form className="question-edit-form" onSubmit={updateQuestion}>
            ...
            const isAnswerOpen = Boolean(question.answer && openAnswerIds.has(question.id));
            ...
            {question.answer && <button className="question-answer-toggle" type="button" aria-expanded={isAnswerOpen} aria-controls={answerId} onClick={() => toggleAnswer(question.id)}><span>{isAnswerOpen ? "답변 접기" : "답변 보기"}</span><CaretDown size={18} weight="bold" /></button>}
            {question.answer && isAnswerOpen && <div className="question-answer" id={answerId}><span>A</span><div><strong>DROP ROOM 답변</strong><p>{question.answer}</p>{question.answeredAt && <small>{formatDate(question.answeredAt)}</small>}</div></div>}
